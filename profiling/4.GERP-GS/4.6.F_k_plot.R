...
x = factor(file, levels=unique(tolower(med2[order(med2$phph),]$trait)))
           
p1 <- ggplot(out, aes(x=factor(file, levels=unique(tolower(med2[order(med2$phph),]$trait))), y=num, fill=k)) + 
  geom_bar(position=position_dodge(), stat="identity") +
  xlab("") +
  ylab("Number of Loci") +
  ggtitle("") +
  labs(fill="Degree of Dominance (k)") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, size=12))

p2 <- ggplot(out2, aes(x=factor(file, levels=unique(tolower(med2[order(med2$phph),]$trait))), y=h2, fill=eff)) + 
  geom_bar(position=position_dodge(), stat="identity") +
  xlab("") +
