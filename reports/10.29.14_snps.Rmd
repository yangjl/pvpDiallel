...
# SNP for founder lines

The following part of work was done on `farm`.
First of all, SNPs (N=10,991) on chr0 were removed. A total number of 13,771,818 for the diallel founder lines were used for the following study.


```{r, eval=F}
### convert data to density SNP format (DSF)
source("munge/2.SNP_filter/2.A.1_SNP_founders.R")
```
Then, run the following `snpfrq` python script to get the SNP frequency and missing rate of the founder lines.
```
#snpfrq is a function in my zmSNPtools repo: https://github.com/yangjl/zmSNPtools
cd /home/jolyang/Documents/pvpDiallel/largedata/SNP
snpfrq -i allsnps_13.7m.dsf2 -s 5 -e 16 -m NH -a 0 -b 1 -o allsnps_frq.out
```

\graphicspath{{../graphs/}}
\begin{figure}
\includegraphics{mafmissing.pdf}
\caption{Minor allele frequency and missing rate of the about 14 million SNP on founder lines}
\label{fig:mfig1}
\end{figure}

```
#SNP filtering based on MAF<0.1 and SNP missing rate >0.5
cd /home/jolyang/Documents/pvpDiallel/largedata/SNP
snpfrq -i allsnps_13.7m.dsf2 -s 5 -e 16 -m H -a 0.1 -b 0.5 -o allsnps_frq.out
```

After filtering, `10,320,715` SNPs were remaining. These SNPs were then projected onto the F1 in the diallel population.
...
```{r, eval=F, fig.margin = TRUE, fig.cap = "Sepal length vs. petal length, colored by species"}
library(ggplot2)
...
```{r, eval=F, fig.width = 10, fig.height = 2, fig.fullwidth = TRUE, fig.cap = "Full width figure"}
...
```{r, eval=F, fig.cap = "Another figure"}
