...
\articletype{inv} 
\newcommand{\X}{\textcolor{red}{\bf X\,}} \newcommand{\jri}[1]{\textcolor{blue}{ \emph{ #1}} }   \title{Utilizing evolutionary conservation information to improve prediction accuracy in genomic selection}
...
\DIFaddbegin \jri{abstract talks GS and prediction, intro talks deleterious alleles and heterosis. i like the intro spin better. should we go with that (and re-write the abstract accordingly)?}
...
\textbf{(A)} Twelve maize inbred lines were selected and crossed in a half diallel. \DIFdelbegin \DIFdel{Ten of these (LH1, LH123HT, LH82, PH207, 4676A, PHG39, PHG47, PHG84, PHJ40, PHZ51) are proprietary inbreds that have expired from Plant Variety Protection (PVP) and represent the lineage of key heterotic germplasm pools used in present-day commercial corn hybrids. Two of them are important public inbreds, B73 and Mo17. }\DIFdelend \textbf{(B)} \DIFdelbegin \DIFdel{Phenotypic data were collected for anthesis-silking interval (ASI, in days), days to 50\% pollen shed (DTP), days to 50\% silking (DTS), ear height (EHT, in cm), grain yield adjusted to 15.5\% moisture (GY, in bu/A), plant height (PHT, in cm), and test weight (TW, in pounds). Analyses were carried out on the traits per se as well as percent high parent heterosis (pHPH)}\DIFdelend \DIFaddbegin \DIFadd{Density plots of the phenotypic distributions}\DIFaddend .
}
\label{fig:pvp-pheno}
