...
%GERP
We used genome-wide estimates of evolutionary constraint, as measured by GERP \citep{Davydov2010}, from \citep{rodgers2015recombination}. Haplotype blocks were then weighted by the GERP scores of their SNPs, with SNPs occurring at sites with GERP scores >0 considered as deleterious. To incorporate the conservation information into IBD blocks, SNPs falling into a given IBD block were added up using their estimated GERP scores. The sum of the score for a given IBD block were used as the conservation estimates of the IBD block. This estimation was calculated using a python script gerpIBD (\url{https://github.com/yangjl/zmSNPtools}) with both additive and dominant modes of inheritance. Under the additive model, 2 x GERP score was assigned to the homozygous loci with non-reference SNP calls; 1 x GERP score was assigned to the heterozygous loci; and 0 was assigned to the homozygous loci with reference SNP calls. Under the dominant model, 1 x GERP score was assigned to both the homozygous loci with non-reference SNP calls and heterozygous loci; 0 was assigned to homozygous loci with reference SNP calls.
...
\caption{Haplotype block identification using an IBD approach. In the upper panel, regions in red color are IBD blocks identified by pairwise comparison of the two parental lines of a hybrid. The vertical dashed define haplotype blocks. In the lower panel, hybrid genotypes at each block are represented as heterozygous (0) or homozygous (1) for each haplotype block.}
\label{fig:defineibd}
\end{figure}
%%%%%%%% ------------- %%%%%%%%%%%
