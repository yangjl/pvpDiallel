...
$\varepsilon$, the model residuals. \jri{perhaps clarify "year" instead of "environment"?} \yang{agreed and changed!}
...
%IBD
We used the fastIBD method implemented in BEAGLE \citep{Browning2009,Browning2011} to impute missing data and identify regions of identity by descent (IBD) between the 12 inbred lines. We then defined haplotype blocks as contiguous regions within which there were no IBD break points across all pairwise comparisons of the parental lines (Figure \ref{fig:defineibd}). \jri{a supplemental figure here would help} \yang{see Figure S2}
...
\caption{Haplotype blocks identification using IBD approach. In the upper panel, regions in red color are IBD blocks identified by pairwise comparison of the two founder lines. The vertical dashed lines were used to define the haplotype blocks. In the lower panel, coding system for the identified haplotype blocks.}
\label{fig:defineibd}
\end{figure}
%%%%%%%% ------------- %%%%%%%%%%%
