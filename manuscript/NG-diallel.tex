...
Complementation of recessive deleterious alleles, however, remains the simplest genetic explanation \citep{Charlesworth2009}, and one that is supported by considerable empirical evidence \citex \citep{xiao1995dominance}.
...
As shown in \textbf{Figure \ref{fig:pheno}a}, BLUE values were normally distributed (Shapiro-Wilk normality test \emph{P} values $>0.05$). 
...
Using the parental phenotypic data, we then estimated best-parent heterosis (BPH) for each trait (\textbf{Figure \ref{fig:pheno}b}).  
...
\begin{figure*}[tbh]   
  \begin{center}
   ...
   \renewcommand{\baselinestretch}{0.9}
   \vspace{-3mm}
   \caption{({\bfseries a-c}) Genomic features of GERP scores. \textbf{(a)} Histogram of GERP scores at $\sim$1.3 million SNP sites. \textbf{(b)} Plot of average GERP scores in bins (bin size = 0.01) of minor allele frequency (MAF). Red and grey lines define the regression and its 95\% confidence interval. \textbf{(c)} Mean GERP scores every cM across 10 maize chromosomes.} 
\vspace{-4mm}
    \label{fig:gerp}
  \end{center}
\end{figure*}
...
\subsection*{Partitioning the phenotypic variance}
