...
More than 86 million bp of the maize reference genome were annotated as conserved\citep{rodgers2015recombination}, with GERP scores $>0$. Nonetheless, 506,898 (or 0.6\%) of these sites were found to segregate among the 12 inbred parents of our diallel (\textbf{Figure \ref{fig:gerpmaf}A} and \textbf{Supplementary Figure 1}).
On average, each inbred parent carried 215,809 putative deleterious variants, ranged from 186,708 of PHG35 to 227,681 of LH123HT. Mo17 carried 225,830 putative deleterious variants, ranking as the second most inbred. \yang{deleterious with GCA} \yang{to do: no pairwise complementation} 
\yang{to do 2:, cm variation} \yang{draft: correlation of the number of deleterious complementation and SCA, MPH and trait per se}
\yang{mean and variation of the deleterious variants per cM}
The minor allele frequency of SNPs at conserved sites was negatively correlated with GERP score (\textbf{Figure \ref{fig:gerpmaf}B}; \emph{P} value $<$ 0.05, \emph{r} = $-0.8$), consistent with the idea that variants at sites with more positive GERP scores are more deleterious and more strongly impacted by purifying selection.
...
To alleviate this limitation, we conceived a haplotype-based genomic selection approach in which we use estimates of evolutionary constraint across the genome to sum the individual effects of deleterious alleles within IBD blocks under additive, dominant and incomplete dominant models (see \textbf{Methods and Supplementary Figure 2}). %\ref{fig:gerpibd}). 
...
\subsection*{Incomplete dominant explained dosage effects for heterosis}

We used GBLUP (\citex) fit with \X kinship matrix. It enabled us to estimate the additive and dominant effect of each SNPs. We defined the degree of dominance (dd) by using the dominant effect divided by the additive effect 
