...
The phenomenon of heterosis or hybrid vigor, referring to the superior performance of hybrids as compared to their parents, was initially documented by Darwin and rediscovered by Shull and East in 1908 \cite{Duvick2001}. This phenomenon has been observed for many species across taxa, from yeast \cite{Shapira2014} to vertebrates \cite{Gama2013}. Although heterosis has been broadly applied for food production and has been extensively studied by many researchers, the underlying mechanism controlling for it remains unclear. Recent studies indicated that the complementation of the deleterious alleles, which fit the classical dominance genetic model, may play an important role in determining heterosis \cite{Charlesworth2009}. 

Deleterious alleles were arisen from new mutations during meiosis. In maize, about 90 new mutations were generated per meiosis \cite{Clark2005}, majority of which were deemed to be deleterious according to empirical estimates \cite{Joseph2004}. In a natural outcross population, the negative effects on fitness of these deleterious alleles make them subject to be selected against. Therefore, deleterious alleles were mainly maintained in a low frequency \cite{Eyre-Walker2007}. 
...
In maize, the total number of these mildly deleterious mutations remain substantial because of the exponential growth of population size after domestication. The modern breeding aims to remove these deleterious mutations and pyramiding beneficial alleles for agronomical purposes. In practice, the relatively homogeneous maize germplasm pool was artificially divided into different heterotic groups \cite{Heerwaarden2012}. It enabled the germplasm improvement could be conducted in a parallel fashion.    

%pesudo-overdominance model
The removing of deleterious mutations in low recombination regions or in tightly linked regions are less effective. For example, residual heterozygosity correlates negatively with recombination \citep{Gore2009, McMullen2009} and the low recombination is effective over long time scales (Haddrill et al. 2007 Genome Biology). As a consequence, the deleterious alleles would be accumulated in the pericentromeric region and the vigorous performace could be only realized by combining two sets of beneficial allels in repulsion state, thus lead to pesudo-overdominance. Many studies have found some evidences to support this hypothesis, such as, QTL for heterosis enriched in centromeric regions (Lariepe et al, 2012 Genetics) and haplotype blocks in low recombination maintain heterosis (Duvick 2005, Advances in agronomy)


%\textbf{Prediction approach for deleterious alleles} 
Previous study observed that an excess of deleterious SNPs enriched in GWAS set. But, Low Fst and few fixed deleterious variants among groups. 
...
In this study, the diallel experimental design enabled us to invistigate the mode of inheritance of the deleterious alleles in genome wide. We use GERP and SIFT MAPP to quantify the evolutionary conservation. GERP can give conservation score of every basepair. In addition to the enrichment analysis, a genomic selection approach was employed with the conservation information incorporated in the model. Our results suggested that the prediction accuarcies were significantly increased compared to circular shuffled conservation scores. But the magtidue of the improvement dependents on the trait and mode of inheritance.

Because most of the deleterious allele are rare, their effects are hard to be detected. To facilitate the detection of deleterious SNPs and ultize it for the breeding, a genomic selection appoach was taken in this study. a evolutionary constraint score was estimated. With a genomic selection appoach, we compared the prediction accuracy with GERP. The results demonstrate that ... 

We therefore, take a genomic enabled prediction approach. 
...
previous show deleterious in GWAS set. Here we introduce it to the genome-wide level.
